...
namespace BlackCat\Database\Packages\RbacUserPermissions\Joins;
 ...
 */
final class RbacUserPermissionsJoins {
    ...
    /**
     * FK: rbac_user_permissions -> users
    ...
    /**
     * FK: rbac_user_permissions -> permissions
     ...
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinUsersByGrantedBy(string $alias = 't', string $as = 'j2'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_users AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.granted_by' . ' ', []];
    }
    /**
     * FK: rbac_user_permissions -> tenants
     * LEFT JOIN vw_tenants AS $as ON $as.id = $alias.tenant_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinTenants(string $alias = 't', string $as = 'j3'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
