...
namespace BlackCat\Database\Packages\RbacUserPermissions\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RbacUserPermissions\Definitions;
use BlackCat\Database\Packages\RbacUserPermissions\Criteria;
use BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto as Dto;
use BlackCat\Database\Packages\RbacUserPermissions\Mapper\RbacUserPermissionDtoMapper as RowMapper;
...
final class RbacUserPermissionRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RbacUserPermissions\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'user_id', 'permission_id', 'tenant_id', 'scope' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RbacUserPermissions\Definitions::table());
            return $bulk->upsertMany($rows, [ 'user_id', 'permission_id', 'tenant_id', 'scope' ], [ 'effect', 'granted_by', 'granted_at', 'expires_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto|null */
    public function getByUserIdAndPermissionIdAndTenantIdAndScope(int $userId, int $permissionId, int $tenantId, string $scope, bool $asDto = false): array|\BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto|null {
        $row = $this->getByUnique([ 'user_id' => $userId, 'permission_id' => $permissionId, 'tenant_id' => $tenantId, 'scope' => $scope ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RbacUserPermissions\Mapper\RbacUserPermissionDtoMapper::fromRow($row);
    }
    public function existsByUserIdAndPermissionIdAndTenantIdAndScope(int $userId, int $permissionId, int $tenantId, string $scope): bool {
        $where = 't.' . Ident::q($this->db, 'user_id') . ' = :uniq_user_id' . ' AND ' . 't.' . Ident::q($this->db, 'permission_id') . ' = :uniq_permission_id' . ' AND ' . 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'scope') . ' = :uniq_scope';
        return $this->exists($where, [ 'uniq_user_id' => $userId, 'uniq_permission_id' => $permissionId, 'uniq_tenant_id' => $tenantId, 'uniq_scope' => $scope ]);
    }
    /** @return int|string|null */
    public function getIdByUserIdAndPermissionIdAndTenantIdAndScope(int $userId, int $permissionId, int $tenantId, string $scope) {
        $row = $this->getByUserIdAndPermissionIdAndTenantIdAndScope($userId, $permissionId, $tenantId, $scope);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto|null {
