...
use BlackCat\Database\Packages\RbacUserPermissions\Repository\RbacUserPermissionRepositoryInterface;
...
class RbacUserPermissionRepository implements RbacUserPermissionRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'user_id', 'permission_id', 'tenant_id', 'scope' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RbacUserPermissions\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'effect', 'granted_by', 'granted_at', 'expires_at' ]);
          return count($rows);
        }
    ...
    public function getByUserIdAndPermissionIdAndTenantIdAndScope(int $userId, int $permissionId, int $tenantId, string $scope, bool $asDto = false): array|\BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto|null {
        return $this->getByUnique([ 'user_id' => $userId, 'permission_id' => $permissionId, 'tenant_id' => $tenantId, 'scope' => $scope ], $asDto);
    }
    ...
    public function getIdByUserIdAndPermissionIdAndTenantIdAndScope(int $userId, int $permissionId, int $tenantId, string $scope) {
        $row = $this->getByUserIdAndPermissionIdAndTenantIdAndScope($userId, $permissionId, $tenantId, $scope, false);
