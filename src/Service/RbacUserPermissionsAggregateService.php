...
namespace BlackCat\Database\Packages\RbacUserPermissions\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RbacUserPermissions\Dto\RbacUserPermissionDto;
use BlackCat\Database\Packages\RbacUserPermissions\Mapper\RbacUserPermissionDtoMapper;
 ...
 */
final class RbacUserPermissionsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RbacUserPermissionRepository $rbacUserPermissionRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rbac_user_permissions' => $this->rbacUserPermissionRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RbacUserPermissions\ModuleException("Record {$idStr} not found for locking.");
            }
